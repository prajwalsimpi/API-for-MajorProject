...
router.post('/login', async (req,res) => {
    ...
    const payloadsend = {username: user.username, userType: user.userType, id: user._id, displayname: user.displayname};
    const token = jwt.sign(payloadsend, process.env.TOKEN_SECRET);
    //Attach the token to the header in the auth-token field
    res.header('auth-token', token).json({token: token, payload: payloadsend});
